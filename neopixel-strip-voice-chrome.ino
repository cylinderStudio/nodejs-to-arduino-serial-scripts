...
const uint16_t SPREAD_INTERVAL = 25;
const uint16_t LEFT_LIMIT = 40;
uint16_t left_count = 0;
const uint16_t RIGHT_LIMIT = 119;
uint16_t right_count = 119;
...
void doLights(String anim) {
  ...
  // TO DO: if showing lights
  if (anim == "solid") {
      setColor(current_color);
      strip.show();
      delay(100);
  } else if (anim == "listening") {
    currentMillis = millis();
    if (currentMillis - previousMillis > SPREAD_INTERVAL) {
      previousMillis = currentMillis;

      setColor(BLUE);

      if (left_count > 0 && left_count < LEFT_LIMIT) {
        for (uint8_t i=left_count; i<right_count; i++) {
    ...
    if (currentMillis - previousMillis > BLINK_INTERVAL) {
      ...
      for (uint8_t i=0; i<6; i++) {
        strip.setPixelColor(i,blink_color);
      }
      ...
      for (uint8_t k=12; k<18; k++) {
        strip.setPixelColor(k,GREEN);
      }

      for (uint8_t m=18; m<PIXELS; m++) {
        strip.setPixelColor(m,BLACK);
      }
